...
export async function render(container, params) {
  ...
  if (!instanceId) { container.innerHTML = '<div class="container-xl py-4"><div class="alert alert-warning">ID istanza mancante.</div></div>'; return; }
        ...
        <div class="row align-items-center">
          <div class="col-auto">
            <a href="#instances" class="btn btn-sm btn-ghost-secondary">
              <i class="ti ti-arrow-left me-1"></i>Fleet
            </a>
          </div>
          <div class="col">
          ...
          <div class="col-auto">
            <button class="btn btn-outline-secondary btn-sm me-2" id="exec-info-btn">
              <i class="ti ti-refresh me-1"></i>Aggiorna info
            </button>
          </div>
        </div>
      </div>
    ...
    <div class="page-body">
      <div class="container-xl">
        ...
        <div class="row row-cards mb-4">
          ...
          <div class="col-4">${gauge('Disco', disk, 'text-orange')}</div>
        </div>
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header d-flex align-items-center">
                <span>Telemetria (ultime 6h)</span>
                <div class="ms-auto">
        ...
        <div class="row row-cards">
          <div class="col-md-6">
            <div class="card">
              <div class="card-header"><h3 class="card-title">Informazioni</h3></div>
              <div class="card-body">
                <dl class="row mb-0">
                  <dt class="col-5">ID</dt><dd class="col-7 text-mono small">${inst.id}</dd>
                  <dt class="col-5">Fingerprint</dt><dd class="col-7 text-mono small">${inst.fingerprint}</dd>
                  <dt class="col-5">Stato enrollment</dt><dd class="col-7">${inst.enrollment_status}</dd>
                  <dt class="col-5">Gruppo</dt><dd class="col-7">${inst.group_id || '—'}</dd>
                  <dt class="col-5">Tags</dt><dd class="col-7">${(inst.tags || []).map(t=>`<span class="badge bg-azure-lt">${t}</span>`).join(' ') || '—'}</dd>
                  <dt class="col-5">Note</dt><dd class="col-7">${inst.notes || '—'}</dd>
                </dl>
              </div>
            </div>
          </div>
          <div class="col-md-6">
            <div class="card">
              ...
              <div class="card-body d-flex flex-column gap-2">
                <button class="btn btn-outline-primary" data-cmd="backup.run" data-label="Avvia backup">
                  <i class="ti ti-database-export me-2"></i>Avvia backup
                </button>
                <button class="btn btn-outline-warning" data-cmd="info" data-label="Ricarica info">
                  <i class="ti ti-info-circle me-2"></i>Ricarica info agente
                </button>
              </div>
            </div>
          </div>
      ...
      try {
        ...
        if (result.success || result.status === 'done' || result.status === 'queued') {
          ...
          inner.textContent = result.status === 'queued'
            ? 'Agente offline — comando in coda'
            : `Completato: ${JSON.stringify(result.result || {})}`;
        } else {
          inner.className = 'alert alert-danger';
          inner.textContent = result.error || 'Errore';
        }
      } catch (e) {
        inner.className = 'alert alert-danger';
        inner.textContent = e.detail || 'Errore';
      } finally {
        btn.disabled = false;
      }
    });
...
function drawTelemetryChart(el, rows) {
  if (!rows || !rows.length) {
    el.innerHTML = '<div class="text-muted text-center py-3">Nessun dato telemetria</div>';
    return;
  }
  ...
  const options = {
    series: [
      ...
      { name: 'Disco %', data: rows.map((r, i) => [labels[i], parseFloat(r.disk_percent.toFixed(1))]) },
    ],
